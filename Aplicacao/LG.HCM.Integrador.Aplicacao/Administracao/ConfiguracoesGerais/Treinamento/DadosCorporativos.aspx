...
<head id="Head1" runat="server">
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
            ...
            var inputIntegracaoTreinamentoTokenClientSecret = $('#IntegracaoTreinamentoTokenClientSecret');
            var vlrPadrao = inputIntegracaoTreinamentoTokenClientSecret.val();

            inputIntegracaoTreinamentoTokenClientSecret.on('click', function () {
                if (inputIntegracaoTreinamentoTokenClientSecret.val() === vlrPadrao) {
                    inputIntegracaoTreinamentoTokenClientSecret.val('');
                }
            });

            $(document).on('click', function (event) {
                if (!$(event.target).closest(inputIntegracaoTreinamentoTokenClientSecret).length) {
                    if (inputIntegracaoTreinamentoTokenClientSecret.val() === '') {
                        inputIntegracaoTreinamentoTokenClientSecret.val(vlrPadrao);
                    }
                }
            });
		});
		...
		var DadosCorporativos = {
			...
			salvar: function (pop, form, param, window) {
				w3.loading(true);
				w3.ajax("SalvarDadosCorporativos", param,
					function (retorno) {
						w3.loading(false);
						if (retorno.OK) {
							w3Form.clearNote();
