...
<head id="Head1" runat="server">
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
            ...
            var inputIntegracaoClaveChave = $('#IntegracaoClaveChave');
            var vlrPadrao = inputIntegracaoClaveChave.val();

            inputIntegracaoClaveChave.on('click', function () {
                if (inputIntegracaoClaveChave.val() === vlrPadrao) {
                    inputIntegracaoClaveChave.val('');
                }
            });

            $(document).on('click', function (event) {
                if (!$(event.target).closest(inputIntegracaoClaveChave).length) {
                    if (inputIntegracaoClaveChave.val() === '') {
                        inputIntegracaoClaveChave.val(vlrPadrao);
                    }
                }
            });
		});
